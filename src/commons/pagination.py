...
def extract_pagination(page=None, per_page=None, **request_args):
    page = int(page) if page is not None else int(os.getenv("DEFAULT_PAGE_NUMBER"))
    per_page = int(per_page) if per_page is not None else int(os.getenv("DEFAULT_PAGE_SIZE"))
